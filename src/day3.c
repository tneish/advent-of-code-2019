...
static int add_or_increment(dict *ht, const void *key, int wire, unsigned int latency) 
{
    struct intIntKey *_key = (struct intIntKey *)key;
    struct coord_val v;
    struct coord_val *pv;
    dictEntry *de;

    // Update coordinate dictionary
    if ((de = dictFind(ht, _key)) == NULL) {
        // New entry
        memset(&v, 0, sizeof(v));
        v.wire_present[wire] = 1;
        v.latency[wire] = latency;
        if (dictAdd(ht, _key, &v) != DICT_OK) {
            printf("Error adding entry to dictionary!\n");
            return 1;
        }
    } else {
        // Already one or more wires at this co-ordinate.
        pv = dictGetVal(de);
        pv->wire_present[wire] = 1;
        pv->latency[wire] = latency;
    }
    return 0;
...
void day3(const char **wires, int num_wires) 
{
    struct intIntKey current_coord;  
    int latency                             = 0;
    dict* number_of_wires_at_coordinates    = dictCreate(&int_int_dict,NULL);
    const char delim[]                 = ",";
    ...
    dictIterator *it;
    dictEntry *de;
    int found_one_or_more_intersections = 0;

    // NB: Intersection of wires at "central port" is not counted.

    /* ------ Record number of wires at a given position in a hash-map ----- */
    // For each wire
    for (int w = 0; w <= num_wires - 1; w++) {
        r = strdup(wires[w]);
        end = r;
        if (DEBUG) printf("New wire: %s.\n", r);
        ...
        while (tk != NULL) {
            // Get number of moves
            const char *cnmoves = strdup(tk + 1);
            int nmoves = atoi(cnmoves);            
            if (DEBUG) printf("New direction: %s. %d moves.\n", tk, nmoves);
            ...
            for (int m = 0; m < nmoves; m++) {
                ...
                latency++;
                (void)add_or_increment(number_of_wires_at_coordinates, &current_coord, w, latency);
            }
            tk = strsep(&end, delim);
        }
        free(r);  
    ...
    /* --- Find all positions with more wires than 1 (e.g. intersections) -- */
    // For each coordinate in dictionary
    it = dictGetSafeIterator(number_of_wires_at_coordinates);
    struct intIntKey *k;
    struct coord_val *pv;
    min_distance = 0;
    min_distance -= 1;  // max unsigned int
    min_latency = 0;
    min_latency -= 1;
    while ((de = dictNext(it)) != NULL) {
        k = dictGetKey(de);
        pv = dictGetVal(de);
        if (DEBUG) printf("%d, %d, %u, %u\n", k->x, k->y, pv->wire_present[0], pv->wire_present[1]);
        if (pv->wire_present[0] && pv->wire_present[1]) {
            // Could be intersection of two wires.
            struct intIntKey k_left = {k->x - 1, k->y};
            struct intIntKey k_right = {k->x + 1, k->y};
            struct intIntKey k_up = {k->x, k->y + 1};
            struct intIntKey k_down = {k->x, k->y - 1};
            if ((dictFind(number_of_wires_at_coordinates, &k_left) != NULL) &&
                (dictFind(number_of_wires_at_coordinates, &k_right) != NULL) &&
                (dictFind(number_of_wires_at_coordinates, &k_up) != NULL) &&
                (dictFind(number_of_wires_at_coordinates, &k_down) != NULL)) {
                // Is an intersection.
                found_one_or_more_intersections = 1;
                // Calculate Manhattan distance from central-port, and total latency
                unsigned int manhattan_dist = abs(k->x) + abs(k->y);
                unsigned int total_latency = pv->latency[0] + pv->latency[1];
                if (DEBUG) printf("Found intersection with manhattan distance: %u, and total latency: %u\n", manhattan_dist, total_latency);
                if (manhattan_dist < min_distance) {
                    if (DEBUG) printf("New minimum manhattan distance.\n");
                    min_distance = manhattan_dist;
                }
                if (total_latency < min_latency) {
                    if (DEBUG) printf("New minimum total latency.\n");
    ...
    if (found_one_or_more_intersections) {
        printf("Closest intersection has manhattan distance %u and total latency %u from central port.\n", min_distance, min_latency);
    } else {
